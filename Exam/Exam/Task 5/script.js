...
function displayUsersInTable(usersArray, tableId) {
  ...
  usersArray.forEach(user => {
    ...
    row.innerHTML = `
      ...
      <td>${user.name}</td>
      <td>${user.age}</td>
      <td>${user.hasDog ? 'Yes' : 'No'}</td>
    `;
    tableBody.appendChild(row);
  });
}
const dogOwners = filterDogOwners(users);
displayUsersInTable(dogOwners, 'dogOwnersTable');

const adults = filterAdults(users);
displayUsersInTable(adults, 'adultsTable');